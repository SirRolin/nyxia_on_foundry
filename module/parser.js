...
import { TokenTypes, Tokenizer} from "./tokenizer.js";
export class Parser {
  parse(input) {
    this.input = input
  ...
  defined_functions(id, args) {
    switch (id) {
      ...
      case 'sum':
        console.log(`${id} : arguments(${args})`)
        return 'sand';
      default:
        throw new Error(`Invalid operation: ${id} : arguments(${args})`)
    }
  }
  ...
  FunctionExpression() {
    ...
    return this.defined_functions(id, expression)
  }
}
